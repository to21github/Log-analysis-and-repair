...
def check_environment(host, db_size, core_log_exists):
    ...
    if disk_total and disk_free >= 0:
        ...
        # 磁盘阈值口径与「系统状态监控」插件统一：百分比阈值 + 绝对下限，任一触发即报警
        # （用量 >90% 或剩余 <500MB 判严重；用量 >75% 或剩余 <2GB 判警告）
        used_pct = disk_used * 100.0 / disk_total
        if disk_free < 500 * 1024 ** 2 or used_pct > 90:
            issues.append({
                "id": "disk_critical",
                ...
                "target": "host",
                "count": 1,
                "samples": ["总容量 %.1f GB，已用 %.1f GB（已用 %d%%）"
                            % (disk_total / GB, disk_used / GB, round(used_pct))],
        ...
        elif disk_free < 2 * GB or used_pct > 75:
            issues.append({
                "id": "disk_low",
