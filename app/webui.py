...
class WebUI:
    ...
    def __init__(self, port, state, trigger_scan, read_report, repair_one=None):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                # 写操作仅接受经 Ingress 反代转发的请求（带 X-Hass-* / X-Forwarded-*
                # 特征头），防止同网络内其他容器直连端口触发扫描 / 修复
                if not any(self.headers.get(h) for h in
                           ("X-Hass-Source", "X-Forwarded-Host",
                            "X-Forwarded-For", "X-Hass-Ingress-Id")):
