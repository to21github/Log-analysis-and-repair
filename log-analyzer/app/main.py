...
WEB_PORT = 8099  # 与 config.yaml 的 ingress_port 保持一致
...
class App:
    def __init__(self):
        ...
        self.ui = webui.WebUI(
            ...
            read_history=reporter.read_history,
        )
    ...
    def serve_forever(self):
        ...
        while True:
            ...
            self.state["next_scan"] = datetime.fromtimestamp(
                time.time() + interval).strftime("%Y-%m-%d %H:%M:%S")
            time.sleep(interval)
...
if __name__ == "__main__":
    print("[日志分析] 插件启动，版本 1.5.0（手动修复模式）")
    App().serve_forever()
